...
`Environments / prd` に値を置く運用にする場合は、対象 job に `environment: prd` を付けます。
...
* `R2_PUBLIC_BASE_URL`: （例: `https://pub-xxxx.r2.dev`）

---

### 3. CD ワークフローへの組み込み

別 workflow を増やすのではなく、既存の `cd-clipor.yml` に `upload_cloudflare_r2` ジョブを追加します。`release` ジョブ完了後に同じ build artifact を再取得し、`.msi` と `.exe` のみを R2 のタグ別ディレクトリへ転送します。その後 `update_release_links` ジョブで、R2 の公開 URL を GitHub Releases 本文へ反映します。
...
さらに `R2_PUBLIC_BASE_URL` が設定されていれば、release 本文に次のような直リンクを動的に追加します。
